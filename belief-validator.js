...
class BeliefValidator {
  ...
  validate(candidate) {
    ...
    // For dynamic subjects (non-reserved), disambiguate but preserve casing
    if (!isReserved && subject.length > 0) {
      // Disambiguate the subject based on context (preserves original format)
      candidate.subject = this.disambiguateSubject(subject, candidate.statement);
    }
    ...
    // 10. First person check (only for beliefs about user/self/assistant)
    const personalSubjects = ['user', 'self', 'assistant'];
    if (personalSubjects.includes(candidate.subject.toLowerCase())) {
      if (!this.firstPersonMarkers.test(stmt)) {
        warnings.push('Personal belief not in first person format');
        score -= 10;
      }
    }
    ...
    // Final check: minimum score threshold (stricter for beliefs than observations)
    const minScore = (candidate.type === 'observation' || candidate.type === 'ephemeral_fact') ? 30 : 50;
    if (score < minScore) {
      errors.push(`Score too low: ${score} (need >= ${minScore} for ${candidate.type})`);
  ...
  normalize(candidate) {
    ...
    // Ensure first person for personal subjects
    const personalSubjects = ['user', 'self', 'assistant'];
    if (personalSubjects.includes(candidate.subject.toLowerCase())) {
